...
const mongodbUrl = 'mongodb+srv://harshad:harshad@cluster0.mr9yg.mongodb.net/myFirstDatabase?retryWrites=true&w=majority';
...
app.post('/login',(req, res) => {
  ...
  MongoClient.connect(mongodbUrl,async function(err, db) {
    if (err) throw err;
    db.db('JCTWARDBANDI').collection('User').findOne({username}, (err, data) => {
      if(err) throw err;
      if(data === null) {
...
app.post('/area', (req, res) => {
  ...
  MongoClient.connect(mongodbUrl,async function(err, db) {
    if (err) throw err;
    db.db('JCTWARDBANDI').collection('User').findOne({_id: userid},(err, data) => {
      if(err) throw err;
      if(data.areas === undefined ) {
        data.areas = [];
      }
      if(data.areas.filter(area => area.nameOfArea === req.body.nameOfArea && area.blockNumber === req.body.blockNumber && area.wardNumber === req.body.wardNumber).length === 0) {
        data.areas.push(req.body);
        delete data._id;
        db.db('JCTWARDBANDI').collection('User').updateOne({_id: userid}, {$set:{...data}}, (err, updatedUser) => {
          if(err) throw err;
          db.close();
...
app.post('/entry',(req,res) => {
  ...
  MongoClient.connect(mongodbUrl, (err, db) => {
    ...
    db.db('JCTWARDBANDI').collection('User').findOne({_id: userId}, (err, data) => {
      if(err) throw err;
      const areaRef = data.areas.filter(area => area.nameOfArea === nameOfArea && area.blockNumber === blockNumber && area.wardNumber === wardNumber)[0];
